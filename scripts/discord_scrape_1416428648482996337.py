"""指定Discordチャンネル 1416428648482996337 の全スレッドからYouTubeリンクを取得"""
...
def run_js(js_code: str, timeout: int = 15) -> str:
...
def scroll_thread_and_get_youtube() -> list[str]:
    """スレッド内をスクロールして全メッセージからYouTubeリンクを取得"""
    all_links = []
    seen = set()
    ...
    for i in range(10):
        ...
        if i < 9:
            ...
            run_js(
                r"""(function() {
                var el = document.querySelector('[class*="messagesWrapper_"]');
                if (el) {
                    var scroller = el.querySelector('[class*="scroller_"]');
...
def main():
    ...
    # スクロールして全カード読み込み（過去の投稿セクションも含む）
    print("スクロール中（過去の投稿含む）...")
    no_new_count = 0
    for i in range(50):  # 多めにスクロール
        scroll_down()
        time.sleep(1.5)
        cards = get_card_titles()
        new_count = 0
        ...
        if new_count > 0:
            ...
            no_new_count += 1
            if no_new_count > 8:
                print(f"  スクロール {i+1}: 新規なし x{no_new_count} → 完了")
                break
    ...
    for idx, title in enumerate(all_titles):
        ...
        links = scroll_thread_and_get_youtube()
